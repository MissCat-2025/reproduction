// SmallDeformationIsotropicElasticityThresholdMod.C 修正版
#include "SmallDeformationIsotropicElasticityThresholdMod.h"
#include "RaccoonUtils.h"

registerMooseObject("reproductionApp", SmallDeformationIsotropicElasticityThresholdMod);

InputParameters
SmallDeformationIsotropicElasticityThresholdMod::validParams()
{
...
SmallDeformationIsotropicElasticityThresholdMod::SmallDeformationIsotropicElasticityThresholdMod(
...
ADReal
SmallDeformationIsotropicElasticityThresholdMod::applyThreshold(ADReal psie_active_raw)
{
  if (_use_threshold)
  {
...
ADRankTwoTensor
SmallDeformationIsotropicElasticityThresholdMod::computeStress(const ADRankTwoTensor & strain)
{
  switch (_decomposition)
  {
    ...
    default:
      mooseError("Unknown decomposition type in SmallDeformationIsotropicElasticityThresholdMod");
  }
  return ADRankTwoTensor(); // 不会执行到这里
}

ADRankTwoTensor
SmallDeformationIsotropicElasticityThresholdMod::computeStressNoDecomposition(const ADRankTwoTensor & strain)
{
  // 计算拉梅常数
...
ADRankTwoTensor
SmallDeformationIsotropicElasticityThresholdMod::computeStressSpectralDecomposition(const ADRankTwoTensor & strain)
{
  // 计算拉梅常数
  const ADReal nu = _poissons_ratio[_qp];
  const ADReal E = _youngs_modulus[_qp];
...
ADRankTwoTensor
SmallDeformationIsotropicElasticityThresholdMod::computeStressVolDevDecomposition(const ADRankTwoTensor & strain)
{
  // 计算拉梅常数
  const ADReal nu = _poissons_ratio[_qp];
  const ADReal E = _youngs_modulus[_qp];
