// SmallDeformationIsotropicElasticityThresholdModMax.C 修正版
#include "SmallDeformationIsotropicElasticityThresholdModMax.h"
#include "RaccoonUtils.h"

registerMooseObject("reproductionApp", SmallDeformationIsotropicElasticityThresholdModMax);

InputParameters
SmallDeformationIsotropicElasticityThresholdModMax::validParams()
{
...
SmallDeformationIsotropicElasticityThresholdModMax::SmallDeformationIsotropicElasticityThresholdModMax(
...
void
SmallDeformationIsotropicElasticityThresholdModMax::updateHistoryMax(ADReal psie_active_current)
{
  if (_use_history_max)
  {
...
ADReal
SmallDeformationIsotropicElasticityThresholdModMax::applyThreshold(ADReal psie_active_raw)
{
...
ADRankTwoTensor
SmallDeformationIsotropicElasticityThresholdModMax::computeStress(const ADRankTwoTensor & strain)
{
  switch (_decomposition)
  {
    ...
    default:
      mooseError("Unknown decomposition type in SmallDeformationIsotropicElasticityThresholdModMax");
  }
  return ADRankTwoTensor(); // 不会执行到这里
}

ADRankTwoTensor
SmallDeformationIsotropicElasticityThresholdModMax::computeStressNoDecomposition(const ADRankTwoTensor & strain)
{
  // 计算拉梅常数
  ...
  // 更新历史最大变量
  updateHistoryMax(_psie_active[_qp]);
...
ADRankTwoTensor
SmallDeformationIsotropicElasticityThresholdModMax::computeStressSpectralDecomposition(const ADRankTwoTensor & strain)
{
  // 计算拉梅常数
  const ADReal nu = _poissons_ratio[_qp];
  const ADReal E = _youngs_modulus[_qp];
...
ADRankTwoTensor
SmallDeformationIsotropicElasticityThresholdModMax::computeStressVolDevDecomposition(const ADRankTwoTensor & strain)
{
  // 计算拉梅常数
  const ADReal nu = _poissons_ratio[_qp];
  const ADReal E = _youngs_modulus[_qp];
