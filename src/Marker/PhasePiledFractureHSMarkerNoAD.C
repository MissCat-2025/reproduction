...
#include "PhasePiledFractureHSMarkerNoAD.h"
#include "FEProblem.h"
#include "MooseEnum.h"

registerMooseObject("reproductionApp", PhasePiledFractureHSMarkerNoAD);

InputParameters
PhasePiledFractureHSMarkerNoAD::validParams()
{
...
PhasePiledFractureHSMarkerNoAD::PhasePiledFractureHSMarkerNoAD(const InputParameters & parameters)
  : QuadraturePointMarker(parameters),
    Coupleable(this, false),
    _von_mises(coupledValue("von_mises_variable")),
    _sigma0(getMaterialProperty<Real>("sigma0")),
...
void
PhasePiledFractureHSMarkerNoAD::markerSetup()
{
  // 清理过期的历史数据
...
bool
PhasePiledFractureHSMarkerNoAD::checkDChangeOverTime(Real current_d_value)
{
...
bool
PhasePiledFractureHSMarkerNoAD::checkStressChangeOverTime(Real current_stress_value)
{
  dof_id_type elem_id = _current_elem->id();
...
Marker::MarkerValue
PhasePiledFractureHSMarkerNoAD::computeQpMarker()
{
