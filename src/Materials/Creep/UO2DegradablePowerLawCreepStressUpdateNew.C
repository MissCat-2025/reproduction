...
#include "UO2DegradablePowerLawCreepStressUpdateNew.h"
#include "RaccoonUtils.h"

registerMooseObject("reproductionApp", UO2DegradablePowerLawCreepStressUpdateNew);

InputParameters
UO2DegradablePowerLawCreepStressUpdateNew::validParams()
{
...
UO2DegradablePowerLawCreepStressUpdateNew::UO2DegradablePowerLawCreepStressUpdateNew(
...
void
UO2DegradablePowerLawCreepStressUpdateNew::computeStressInitialize(
...
ADReal
UO2DegradablePowerLawCreepStressUpdateNew::computeResidual(
    const ADReal & effective_trial_stress, const ADReal & scalar)
{
  ...
  // 计算有效应力，考虑退化函数
  ADReal effective_stress;
  if (_use_stress_degradation)
  {
    // 获取应力张量
    const ADRankTwoTensor & stress_tensor = _stress[_qp];
    
    // 取偏应力
    ADRankTwoTensor dev = stress_tensor.deviatoric();
    ...
    ADReal alpha = 2.0 / 3.0 * Np_p.doubleContraction(Np_p);
    
    // α=2/3∥N_p^+ ∥_F^2
    // σ_"eff" ^(n+1)=√((σ_eff^trial-3GΔε ̅^p )^2+(3GΔε ̅^p )^2 (g^2-1)α)
    ADReal term1 = effective_trial_stress - _three_shear_modulus * scalar;
    ...
    if (MetaPhysicL::raw_value(radicand) <= 0.0)
      radicand = 0.0;
    effective_stress = sqrt(radicand);
  }
  else
  {
  ...
  if (_use_transition_stress)
  {
    ...
    else
    {
      ...
      creep_th2 = _a8 * _density_term2 * pow(effective_stress, 4.5) * _exp_Q2;
    }
  }
  else
  {
  ...
  // 辐照蠕变
  ADReal creep_ir = _a7 * _fission_rate * effective_stress * _exp_Q3;
...
ADReal
UO2DegradablePowerLawCreepStressUpdateNew::computeDerivative(
    const ADReal & effective_trial_stress, const ADReal & scalar)
{
  using MetaPhysicL::sqrt;
  using std::sqrt;
  ...
  // 计算有效应力，考虑退化函数
  ADReal effective_stress;
  ADReal d_effective_stress_d_scalar;
  ...
  if (_use_transition_stress)
  {
    ...
    else
    {
      ...
      d_creep_th2_d_stress = 4.5 * _a8 * _density_term2 * pow(effective_stress, 3.5) * _exp_Q2;
    }
  }
  else
  {
...
void
UO2DegradablePowerLawCreepStressUpdateNew::computeStressFinalize(
...
Real
UO2DegradablePowerLawCreepStressUpdateNew::computeStrainEnergyRateDensity(
