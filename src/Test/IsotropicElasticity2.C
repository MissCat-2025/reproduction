// IsotropicElasticity2.C 修正版
#include "IsotropicElasticity2.h"
#include "RaccoonUtils.h"
#include <cmath>

registerMooseObject("reproductionApp", IsotropicElasticity2);
...
computeIsotropicElasticConstants(const ADReal & E,
                                 ...
                                 const IsotropicElasticity2::KinematicAssumption kinematic_assumption,
                                 ADReal & G,
                                 ADReal & lambda,
                                 ADReal & K)
{
  ...
  if (dim == 2)
  {
    if (kinematic_assumption == IsotropicElasticity2::KinematicAssumption::plane_stress)
...
InputParameters
IsotropicElasticity2::validParams()
{
...
IsotropicElasticity2::IsotropicElasticity2(
...
void
IsotropicElasticity2::updateHistoryMax(ADReal psie_active_current)
{
  if (_use_history_max)
  {
...
ADReal
IsotropicElasticity2::applyThreshold(ADReal psie_active_raw)
{
...
ADRankTwoTensor
IsotropicElasticity2::computeStress(const ADRankTwoTensor & strain)
{
  switch (_decomposition)
  {
    ...
    default:
      mooseError("Unknown decomposition type in IsotropicElasticity2");
  }
  return ADRankTwoTensor(); // 不会执行到这里
}

ADReal
IsotropicElasticity2::computeThreeShearModulus()
{
...
ADRankTwoTensor
IsotropicElasticity2::computeStressIntact(const ADRankTwoTensor & strain)
{
  const ADReal nu = _poissons_ratio[_qp];
  const ADReal E = _youngs_modulus[_qp];
  ADReal G;
...
ADRankTwoTensor
IsotropicElasticity2::computeStressNoDecomposition(const ADRankTwoTensor & strain)
{
  const ADReal nu = _poissons_ratio[_qp];
  const ADReal E = _youngs_modulus[_qp];
  ADReal G;
...
ADRankTwoTensor
IsotropicElasticity2::computeStressSpectralDecomposition(const ADRankTwoTensor & strain)
{
  const ADReal nu = _poissons_ratio[_qp];
  const ADReal E = _youngs_modulus[_qp];
  ADReal G;
...
ADRankTwoTensor
IsotropicElasticity2::computeStressVolDevDecomposition(const ADRankTwoTensor & strain)
{
  const ADReal nu = _poissons_ratio[_qp];
  const ADReal E = _youngs_modulus[_qp];
  ADReal G;
...
ADRankTwoTensor
IsotropicElasticity2::computeStressMaxPrincipalDecomposition(const ADRankTwoTensor & strain)
{
