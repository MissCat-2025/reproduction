# === 平面应力版本 (二维截面) ===
# 依据弱平面应力：使用 out_of_plane_strain=strain_zz 与 ADWeakPlaneStress
...
pellet_inner_diameter = 9.90         # 芯块内直径mm10.291
pellet_outer_diameter = 14.10         # 芯块外直径mm14.627
length_scale_paramete = 4e-5
mesh_size = 7.00e-5 #网格尺寸即可
...
[MultiApps]
  [fracture]
    ...
    input_files = '2D_Sub_plane_stress.i'
...
[Adaptivity]
  ...
  [Markers]
    [marker]
      ...
      sigma0 = sigma0
      x1 = 0.0000001 #d变量小于x1时，标记为粗网格
      x2 = 0.0005 #d变量在x1和x2之间时，标记为细网格
      xmax = 0.01 #d变量大于xmax时，一定是细网格
      y1 = 0.35 #vonMises应力小于y1时，标记为粗网格
      y2 = 0.4 #vonMises应力大于y2之间时，标记为细网格
      variable = d
      timeD = 3
      timeStress = 5
