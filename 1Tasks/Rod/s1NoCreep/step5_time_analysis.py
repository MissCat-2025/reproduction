...
def extract_time_info_from_log(log_path, target_times):
    ...
    try:
        ...
        if runtime_match:
            ...
            if timestamps:
                try:
                    ...
                    if elapsed > 0:
                        time_info['total_runtime'] = elapsed
                except Exception:
                    pass
        ...
        # 如果没有显式返回码，根据错误信息和时间步判断状态
        if time_info['return_code'] == -1:
            if errors:
                time_info['convergence_status'] = 'failed'
            elif time_info['time_steps']:
                # 有时间步、无错误信息，且无返回码，视为用户主动中断
                time_info['convergence_status'] = 'interrupted'
...
def generate_summary_report(all_cases, target_times, output_dir):
    ...
    total_runtime_all = sum(runtimes) if runtimes else 0
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"总运行时间: {total_runtime_all:.2f}秒\n")
