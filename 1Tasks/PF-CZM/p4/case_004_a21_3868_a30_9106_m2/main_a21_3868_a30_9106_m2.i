...
[MultiApps]
  [fracture]
    ...
    input_files = 'sub_a21_3868_a30_9106_m2.i'
    cli_args = 'l=${length_scale_paramete};mesh_size=${mesh_size};m=${m};w=${w};a2=${a2};a3=${a3};endTime=${endTime};dtmin=${dtmin};dt=${dt};pellet_inner_diameter=${pellet_inner_diameter};pellet_outer_diameter=${pellet_outer_diameter};dtMax=${dtMax}'
...
[AuxKernels]
  ...
  [./elastic_strain_radial]
    type = ADRankTwoScalarAux
    rank_two_tensor = elastic_strain
    variable = elastic_strain_radial
    scalar_type = RadialStress
    point1 = '0 0 0'
    point2 = '0 0 1'
  [../]
  ...
  []
  [eig_thermal_aux]
    # 计算热应变的迹，它的1/3就是热线应变，可与其它本征应变的进行比较
    type = ADRankTwoScalarAux
    rank_two_tensor = thermal_eigenstrain
    variable = eig_thermal_trace
...
[Materials]
    ...
    [largestPoreSize]
      ...
      expression = 'PS+PS*3.7*(burnup/(0.015+burnup))'
      constant_names = 'PS'
    ...
    [porosity]
      ...
      material_property_names = 'burnup'
      expression = 'p0+p0*4.7*(burnup/(0.01+burnup))'
      constant_names = 'p0'
    ...
    [sigma0_F_factor]
      type = ADDerivativeParsedMaterial
      property_name = sigma0_F_factor
      material_property_names = 'burnup'
      coupled_variables = 'sigma0_F_Density'
      expression = 'if(sigma0_F_Density < 1, max(0.0, 1 - (1 - sigma0_F_Density) * (1 + burnup / (0.01 + burnup))), sigma0_F_Density)'
      output_properties = 'sigma0_F_factor'
      outputs = exodus
      block = pellet
    []
    [sigma0]
      ...
      # coupled_variables = 'T'
      material_property_names = 'porosity largestPoreSize density sigma0_F_factor'
      expression = 'sigma0_F_factor*1.288e9*(largestPoreSize+0.5*GS)^(-0.5)*exp(-0.057*porosity)*(1-2.62*(1-density/10960))^0.5*exp(-1590/8.314/T)'
      constant_names = 'GS T'
      constant_expressions = '${grain_size} ${T_fracture}'
    ...
    #   material_property_names = 'porosity largestPoreSize density'
