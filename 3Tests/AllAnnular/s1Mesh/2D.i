# conda activate moose && mpirun -n 12 /home/yp/projects/reproduction/reproduction-opt -i 2D.i --mesh-only
initial_T = 583.15
EndTime = 150000000
pellet_nu = 0.345
...
[nodeset1]
  ...
  input = rename2
  expression = 'x = 0 & y>(${inner_clad_outer_radius}-${mesh_size}/200) & y<${pellet_inner_radius}+${mesh_size}/200'
  epsilon =  1e-7
  new_nodeset_name = y_axis_point1
[]
[nodeset2]
  type = ParsedGenerateNodeset
  input = nodeset1
  expression = 'x = 0 & y>(${pellet_outer_radius}-${mesh_size}/200) & y<${outer_clad_inner_radius}+${mesh_size}/200'
  epsilon =  1e-7
  new_nodeset_name = y_axis_point2
[]
[nodeset3]
  type = ParsedGenerateNodeset
  input = nodeset2
  expression = 'y = 0 & x>(${inner_clad_outer_radius}-${mesh_size}/200) & x<${pellet_inner_radius}+${mesh_size}/200'
  epsilon =  1e-7
  new_nodeset_name = x_axis_point1
[]
[nodeset4]
  type = ParsedGenerateNodeset
  input = nodeset3
  expression = 'y = 0 & x>(${pellet_outer_radius}-${mesh_size}/200) & x<${outer_clad_inner_radius}+${mesh_size}/200'
  epsilon =  1e-7
  new_nodeset_name = x_axis_point2
[]
[]
...
[BCs]
  ...
  [y_zero_on_y_plane]
    type = DirichletBC
    variable = disp_y
    boundary = 'x_axis_point1 x_axis_point2'
    value = 0
  []
  [x_zero_on_x_plane]
    type = DirichletBC
    variable = disp_x
    boundary = 'y_axis_point1 y_axis_point2'
    value = 0
  []
