...
_default_parameter_matrix = {
    'pellet_critical_energy': [3],
    'PressureFactor': [1e6, 2e6, 3e6, 4e6, 5e6],
}
_env_param_matrix = os.environ.get("STEP1_PARAM_MATRIX")
if _env_param_matrix:
    try:
        parameter_matrix = json.loads(_env_param_matrix)
    except Exception:
        parameter_matrix = _default_parameter_matrix
else:
    parameter_matrix = _default_parameter_matrix
