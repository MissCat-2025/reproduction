...
_default_fields_series = [
    ('d', '相场变量'),
    ('hoop_stress', '环向应力'),
    ('sigma0', '断裂强度'),
]
_env_fields_series = os.environ.get("PV_SERIES_FIELDS") or os.environ.get("PV_FIELDS")
_fields_series = _parse_fields(_env_fields_series, _default_fields_series)
_image_size_series = _parse_image_size()
_output_dir_series = os.environ.get("PV_OUTPUT_DIR_SERIES", "parameter_studies_series")

DEFAULT_CONFIG = ExportConfig(
    target_times=_parse_target_times(),
    field_list=_fields_series,
    image_size=_image_size_series,
    output_dir_name=_output_dir_series
)
    ...
    try:
        ...
        for field, display_name in config.field_list:
            ...
            if expected_outputs and all(os.path.isfile(p) for p in expected_outputs):
                print(f"  已存在{len(expected_outputs)}张目标时间截图，跳过该字段")
                continue
            ...
            for target_time in config.target_times:
                closest_time = closest_times_by_target[target_time]
...
def _iter_case_dirs(studies_dir):
    case_dirs = [
...
def main():
    ...
    studies_subdir = os.environ.get("STUDIES_SUBDIR", "parameter_studies_series")
    ...
    output_dir_name = os.environ.get("PV_OUTPUT_DIR_SERIES", "parameter_studies_series")
    case_dirs = _iter_case_dirs(studies_dir)
    if not case_dirs:
