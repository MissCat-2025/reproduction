...
_default_checkpoint_config = '''
  ...
  []'''
checkpoint_config = os.environ.get("STEP1_CHECKPOINT_CONFIG", _default_checkpoint_config)
# 参数序列定义（“对齐配对”生成）：
# 示例：
# parameter_matrix = {
...
#   'linePower': [1, 2, 3, 4]
# }
_default_parameter_matrix_series = {
    'a2': [3.1748, -0.5, 0.5396842, 1.3868],
    'a3': [0, 0, 0, 0.9106],
    'm': [2.5, 2, 4, 2],
}
_env_param_matrix_series = os.environ.get("STEP1_SERIES_PARAM_MATRIX")
if _env_param_matrix_series:
    try:
        parameter_matrix = json.loads(_env_param_matrix_series)
    except Exception:
        parameter_matrix = _default_parameter_matrix_series
else:
    parameter_matrix = _default_parameter_matrix_series
