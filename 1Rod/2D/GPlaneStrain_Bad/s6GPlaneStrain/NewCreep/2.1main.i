...
[Kernels]
  #力平衡方程
    ...
    [gps_offdiag_disp_x]
      ...
      eigenstrain_names = 'thermal_eigenstrain_reg gps_creep_strain'
      base_name = gps
    []
    [gps_offdiag_disp_y]
      type = GeneralizedPlaneStrainOffDiag
    ...
    [heat_conduction]
      type = HeatConduction
      variable = T
      diffusion_coefficient = thermal_conductivity_reg
      block = pellet
    []
    [hcond_time]
      type = HeatConductionTimeDerivative
      variable = T
      specific_heat = specific_heat_reg
      density_name = density_reg
      block = pellet
    []
    [Fheat_source]
      type = CoupledForce
...
[Materials]
    ...
    [pellet_thermal_conductivity] #新加的！！！！！！！！！！！！！！！！！！！！！！
      ...
      block = pellet
    []
    [pellet_thermal_conductivity_reg]
      type = ParsedMaterial
      property_name = thermal_conductivity_reg
      coupled_variables = 'T'
      expression = '(1)*(100/(7.5408 + 17.692*T/1000 + 3.6142*(T/1000)^2) + 6400/((T/1000)^2.5)*exp(-16.35/(T/1000)))'
      block = pellet
    []
    [pellet_specific_heat]
      ...
      block = pellet
    []
    [pellet_specific_heat_reg]
      type = ParsedMaterial
      property_name = specific_heat_reg
    ...
    [pellet_thermal_eigenstrain]
      ...
      block = pellet
    []
    [pellet_thermal_eigenstrain_reg]
...
[Outputs]
  [my_checkpoint]
    type = Checkpoint
    time_step_interval = 20    # 每5个时间步保存
